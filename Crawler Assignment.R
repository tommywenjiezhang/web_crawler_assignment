...
while (j < num){
  ...
  htmlContext <- content(GET(exploredlink),type="text/html",as="text")
  bodyText<- tryCatch(htmlToText(htmlContext),error=function(cond){return("")})
  ...
  for (val in topicwords)
  {
    if(val %in% bodyText){
      ...
      write.table(tryCatch(htmlToText(htmlContext),error=function(cond){return("")}), 
                  file= path, 
                  quote = FALSE,
                  col.names = FALSE,
                  row.names = FALSE)
